...
class WhenTryingToPushValuesTest {
    ...
    private final Optional<Tuple> second = first.tryPush(SECOND_INPUT);
    private final Optional<Tuple> third = second.isPresent() ? second.get().tryPush(THIRD_INPUT) : Optional.empty();
    private final Optional<Tuple> fourth = third.isPresent() ? third.get().tryPush(FOURTH_INPUT) : Optional.empty();
    private final Optional<Tuple> fifth = fourth.isPresent() ? fourth.get().tryPush(FIFTH_INPUT) : Optional.empty();
    private final Optional<Tuple> sixth = fifth.isPresent() ? fifth.get().tryPush(SIXTH_INPUT) : Optional.empty();
    private final Optional<Tuple> seventh = sixth.isPresent() ? sixth.get().tryPush(SEVENTH_INPUT) : Optional.empty();
    private final Optional<Tuple> eighth = seventh.isPresent() ? seventh.get().tryPush(EIGHTH_INPUT) : Optional.empty();
    private final Optional<Tuple> extra = eighth.isPresent() ? eighth.get().tryPush(EXTRA_INPUT) : Optional.empty();
